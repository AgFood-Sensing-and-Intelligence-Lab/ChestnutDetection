...
if __name__ == '__main__':
    ...
    model = YOLO('/root/yolov11/model/yolo11x.pt')
    ...
    model.train(
        ...
        workers=2,              # 数据加载线程数
        project='/root/yolov11/resultsx_5',  # 输出目录
        name='train',           # 训练名称

        optimizer='AdamW',       # 自动选择优化器 :contentReference[oaicite:3]{index=3}
        lr0=0.0005,               # 初始学习率 0.01 :contentReference[oaicite:4]{index=4}
        lrf=0.01, 
        momentum=0.9,
